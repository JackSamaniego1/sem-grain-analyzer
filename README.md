...
From the Projects page, select two or more lots and choose **Compare** in
the selection bar to open the lot comparison view: mean ASTM G with a 95%
confidence interval for each lot, a colour-coded ΔG matrix, an overlaid
grain-size distribution chart, and a plain-language pass/fail equivalence
verdict against a baseline lot you choose (star icon), with an adjustable
tolerance. Useful for confirming a new lot matches a qualified reference
lot, or spotting a process drift between lots of the same material.
...
- Build the macOS disk image: `bash BUILD_MAC.sh` (PyInstaller + `hdiutil`)
