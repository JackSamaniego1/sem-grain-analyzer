...
1. Quick HIER-01 follow-ups: (a) wizard relabel on Settings change, (b) wizard date-field arrow clip, (c) Excel/PowerPoint export re-verify vs HIER-01 acceptance (hierarchy labels, export basename).
   Then **UI-09** (user request): right-click Delete/Move/Rename on folders & images, per-row select checkboxes, multi-select action bar (Delete · Move to…). See task board.
