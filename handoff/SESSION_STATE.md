...
**Branch:** `v3-dev` · **Last code commit:** `92b6093` (demo workspace HIER-01) · GitHub push still blocked (FND-04).
...
Phases 0–3 complete; most of Phase 4. PySide6 + 3-layer offline guard; black-region fix; ASTM E112 G (core/astm.py); non-destructive grain filters (core/postfilter.py + UI filter card); data layer (trash/restore, CLEAR sentinel); report engine + report designer (exports honour all edits, 4 palettes, custom text); app shell (Projects, Wizard, Analyze, Review, Reports, Settings); SEM info-bar auto-exclusion + metadata calibration (e6d0e1d); HIER-01 data + UI (a916add, b50751d, 1d6df50, 525fc9f, 92b6093); DET-05 + INN-05 integrated; crash-on-close fix; session trash undo. Full suite: **412 passed**.
